        ...
        <div className="team_page">
            ...
            <section className="team_select">
                ...
                <table className="team_select_table">
                    ...
                    {data.members.filter((element)=> data.host.username != element.info.user.username).map((element)=><tr><td>{element.info.user.username}</td><td>{element.info.sid}</td><td>{element.info.major}</td><td>{element.info.user.identity == 'student user' ? "Year " + (year - element.info.admission_year.slice(0,4)) : ''}</td><td>{element.state==="accepted" && <Ti.TiTick className="tick"/>}{element.state==="pending" && <Fa.FaQuestion/>}{element.state==="rejected" && <Im.ImCross className="cross"/>}</td><td>{element.state == "pending" && <button className="accept_button" type="button" onClick={()=>AcceptTeam(element.id, element.info.user.username)}>Accept</button>}</td><td>{element.state == "pending" && <button type="button" className="reject_button" onClick={()=>RejectTeam(element.id, element.info.user.username)}>Reject</button>}</td></tr>)}
                </table>
            </section>
        </div>
        )

...
function Stu_profile() {
  const params=useParams();
    ...
    //get the username, post id and feature from URL
    let name=params.username;
    let id=params.id
    //used to determine which section call this page
    let feature = params.feature

    const [items, setItems] = useState([{}]);

    //get the profile information by the id from params
  useEffect(() => {
  ...
  try{
      ...
      return (
        <><div className="banner">
          ...
          {/*used to determine the section which to be back  */}
